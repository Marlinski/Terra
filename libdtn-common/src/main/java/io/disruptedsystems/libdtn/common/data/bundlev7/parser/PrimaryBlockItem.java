...
public class PrimaryBlockItem implements CborParser.ParseableItem {
    ...
    private static final Logger log = LoggerFactory.getLogger(PrimaryBlockItem.class);
    ...
    public CborParser getItemParser() {
        return CBOR.parser()
                .do_here((p) -> {
                    log.trace( ". preparing primary block Crc");
                ...
                .cbor_parse_custom_item(() -> new EidItem(), (p, t, item) -> {
                    log.trace( ". destination=" + item.eid.toString());
                    bundle.setDestination(item.eid);
                })
                .cbor_parse_custom_item(() -> new EidItem(), (p, t, item) -> {
                    log.trace( ". source=" + item.eid.toString());
                    bundle.setSource(item.eid);
                })
                .cbor_parse_custom_item(() -> new EidItem(), (p, t, item) -> {
                    log.trace( ". reportto=" + item.eid.toString());
                    bundle.setReportTo(item.eid);
                })
                .cbor_open_array(2)
                .cbor_parse_int((p, t, i) -> {
                    log.trace( ". creationTimestamp=" + i);
                    bundle.setCreationTimestamp(i);
                })
                .cbor_parse_int((p, t, i) -> {
                    log.trace( ". sequenceNumber=" + i);
                    bundle.setSequenceNumber(i);
                    log.trace( ". bid=" + bundle.getBid().toString());
                })
                .cbor_parse_int((p, t, i) -> {
                    log.trace( ". lifetime=" + i);
