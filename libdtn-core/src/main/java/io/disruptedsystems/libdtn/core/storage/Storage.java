...
public class Storage extends CoreComponent implements StorageApi {
    ...
    private class CoreBlobFactory extends BaseBlobFactory {
        CoreBlobFactory() {
            enableVolatile(conf.<Integer>get(ConfigurationApi.CoreEntry.VOLATILE_BLOB_STORAGE_MAX_CAPACITY).value());
            enablePersistent("");
        }

        @Override
        public boolean isVolatileEnabled() {
            return conf.<Boolean>get(ConfigurationApi.CoreEntry.COMPONENT_ENABLE_VOLATILE_STORAGE).value();
        }

        @Override
        public boolean isPersistentEnabled() {
            return conf.<Boolean>get(ConfigurationApi.CoreEntry.COMPONENT_ENABLE_SIMPLE_STORAGE).value();
        }
